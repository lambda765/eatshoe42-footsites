...
public class FootsiteTask extends Thread{
	...
	public FootsiteTask(String taskID, Profile testProfile, String sku2, String size2, String site2) {
			...
			try {
				...
				try(Response response = httpClient.newCall(request).execute()){
					...
					if(statusCode == 503) {
						...
						if(response.body().string().contains("backend")) {
							this.log("err","Footsites Backend Block");
							Thread.sleep(3000);
						}
					}
				...
				this.log("err", "Error Getting Session");
			}
		}
	}
			...
			try {
				...
				try(Response response = httpClient.newCall(request).execute()){
					...
					try {
						...
						JSONArray variantAttributes = jsonObject.getJSONArray("variantAttributes");
						System.out.println(variantAttributes.getJSONObject(0).getString("skuLaunchDate"));
						...
						else if(this.size == "random") {
							//this.log("msg", sizeSkus.toString());
						}*/else{
		...
		while(true){
			try(Response response = httpClient.newCall(request).execute()){
				...
				if(statusCode == 200){
						...
						this.log("msg", responseHeaders.get("x-cache"));
						//this.log("msg", responseHeaders.toString());
	...
	public void sendToDiscord() throws ParseException, IOException {
		...
		try(Response response = httpClient.newCall(request).execute()){
			...
			this.log("err", "Error Sending to Discord");
		}
	}
}